...
description: PostgreSQL 批量数据操作专家，基于 PostgresqlAsyncBulk 实现高性能数据操作。**主动用于**：大数据量导入（>1000条）、批量更新、数据同步、ETL 场景、初始化数据。当用户提到"批量"、"导入"、"同步"、"大数据量"、"Excel导入"、"数据迁移"时，必须使用此技能。
---
...
| 方法 | 用途 | 典型场景 | 是否删除数据 |
|-----|-----|---------|-------------|
| `CopyToServer` | 批量插入 | 纯新增数据 | ❌ |
| `CopyToServerWithoutUniqueKey` | 条件插入 | 避免重复插入 | ❌ |
| `MergeToServer` | 存在则更新，不存在则插入 | 数据同步、Upsert | ❌ |
| `UpdateToServer` | 批量更新 | 批量修改字段 | ❌ |
| `MergeAndDeleteToServer` | 完整同步 | 全量同步（源数据覆盖目标） | ✅ |
