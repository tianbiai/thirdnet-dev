...
description: 后台定时任务开发专家，基于 BackgroundRunner 框架生成循环执行的后台任务代码。**主动用于**：定时任务、后台作业、周期性数据处理、数据同步、定时检查。当用户提到"定时"、"周期"、"后台任务"、"Job"、"Worker"、"每隔"、"自动执行"、"BackgroundService"、"调度"、"Cron"、"定时同步"、"后台处理"时，必须使用此技能。
---
