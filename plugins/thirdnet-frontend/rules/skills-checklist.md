...
| `net-mock-api-developer` | ⭐⭐ | 涉及接口字段变更时（新增/修改/删除 Mock API）
...
### net-mock-api-developer ⭐⭐

.NET Mock API 接口开发专家，为前端项目创建本地 Mock 服务，提供与真实后端格式一致的模拟数据。

**核心功能**：
- 创建 Mock API Controller（.NET，无认证/无数据库依赖）
- 定义 Request/Response DTO 模型（命名规范与 thirdnet-backend 一致）
- 返回固定 Mock JSON 数据供前端联调
- 配置 CORS 跨域和 Vite 代理

**何时使用**：涉及接口字段变更时必读（新增/修改/删除 API 请求参数或响应字段）

**项目位置**：`frontend/{项目名}/mockapi/`

**关键规则**：
- 路由格式与真实后端完全一致：`api/{endpoint}/{module}`
- DTO 属性使用 snake_case：`user_name`、`create_time`
- 仅使用 GET/POST 方法
- 响应不包装，直接返回实体 JSON
...
5. net-mock-api-developer      → Mock API 开发（涉及接口变更时）
6. create-adaptable-composable → 创建可复用 Composable（按需）
7. /ui-ux-pro-max              → 页面视觉优化（按需）
```
