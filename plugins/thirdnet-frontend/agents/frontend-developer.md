...
- **TypeScript 强制**：所有前端代码必须使用 `.ts` 扩展名，Vue 组件必须使用 `<script setup lang="ts">`，禁止原生 JS 文件（`.js`）
- **枚举规范**：所有枚举类型使用 `enum` 关键字定义 + JSDoc 注释，禁止 union type 或 const object 替代
...
#### SFC 行数上限（强制，详见 `skills/doc-templates/references/sfc-large-component-refactoring.md`）
...
| 场景 | 默认设计风格 |
|------|--------------|
| Web 端、移动端 APP、H5 | 苹果设计规范（简洁克制、清晰层次、一致性、适度动效），详见 `skills/design-apple/` |
...
### 规则 9：API 策略工厂架构（TypeScript）

系统采用**接口契约的策略工厂模式**，API 接口与 Mock 数据完全解耦。核心原则：API 模块通过 TypeScript 接口定义契约（`IXxxApi`），Real 和 Mock 作为可互换的策略实现，工厂函数根据 `.env` 配置自动选择。
...
| 模式 | 体现 |
|------|------|
| **策略模式** | `IOrderApi` 定义策略接口，`RealOrderApi` 和 `MockOrderApi` 是可互换的策略实现 |
| **简单工厂** | `createOrderApi(): IOrderApi` 根据 `MOCK_ENABLED` 创建具体实例 |
| **适配器模式** | `RealOrderApi` 适配远程 HTTP，`MockOrderApi` 适配本地内存数据，两者适配同一套接口契约 |
...
pages/order/index.vue  ←→  api/modules/app/order.ts         ←→  mock/data/app/order.ts
  引用 orderApi.getList()    枚举 + 类型 + IXxxApi +           import type + import enum
                              Real + Mock + factory            + mockOrderList（字段与类型一致）
```

```typescript
// api/modules/app/order.ts —— 枚举 → 类型 → 接口契约 → Real → Mock → 工厂
...
export interface IOrderApi {
  getOrderList(params: PaginationParams): Promise<PaginatedResponse<OrderItem>>
}
...
// mock/data/app/order.ts —— import type + import enum 保证类型一致
...
export const mockOrderList: OrderItem[] = [
  { order_id: 1, order_no: 'ORD202401001', status: OrderStatusEnum.Paid, amount: 299.00, created_at: '2024-01-15' },
]
```

#### API 规范

- **策略工厂**：每个 API 模块包含 `IXxxApi` 接口 + `RealXxxApi`（适配 HTTP）+ `MockXxxApi`（适配本地数据）+ `createXxxApi()` 工厂函数
...
- **枚举类型**：所有枚举使用 `enum` 关键字 + JSDoc 注释，禁止 union type 替代
...
**无缝切换**：在 `.env` 文件中设置 `VITE_MOCK=true/false`，`config/index.ts` 导出 `MOCK_ENABLED`。工厂函数根据此标志自动选择 Real 或 Mock 实现。**切换只需修改 `.env`，无需改动任何业务代码。**
...
创建 changelog.md 及配套渲染页面（模板：`skills/doc-templates/references/changelog-template.md`）：

| 端 | 路径 |
|------|------|
| **Web** | `frontend/{项目名}/public/changelog.md` + `viewer.html` + `marked.min.js` |
| **小程序** | `frontend/{项目名}/static/changelog.md` + `viewer.html` + `marked.min.js` |

模板参考：`skills/doc-templates/references/markdown-viewer.md`

### 阶段 3：生成项目 Spec

创建 `frontend/{项目名}/spec.md`，模板：`skills/doc-templates/references/project-spec-template.md`
...
frontend/
 └── {项目名}/
     ...
     └── src/
         ...
         │   ├── types/enums.ts       # 跨模块复用的枚举类型（enum + JSDoc）
...
- `skills/design-apple/` - Apple 设计规范（Web + 移动端默认）
...
- `skills/api-typescript-spec/` - API 接口 TypeScript 规范（策略工厂模式 + 接口契约 + 枚举规范）

### 参考文档

- `rules/skills-checklist.md` - 技能检查清单
- `skills/doc-templates/references/sfc-large-component-refactoring.md` - 大文件重构规则
- `skills/doc-templates/references/project-spec-template.md` - 项目规格模板
- `skills/doc-templates/references/page-spec-template.md` - 页面规格模板
- `skills/doc-templates/references/changelog-template.md` - 变更日志模板
- `skills/doc-templates/references/markdown-viewer.md` - 通用 Markdown 查看器
