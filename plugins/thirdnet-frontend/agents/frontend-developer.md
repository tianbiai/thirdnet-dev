...
收到需求"创建一个用户管理页面"时，第一轮 AskUserQuestion 调用：

- Q1: "目标平台？" → [Web (Element Plus), 移动端 (Vant/uniapp), 双平台]
- Q2: "用户管理包含哪些功能？" → [列表+CRUD, 列表+详情, 完整管理（含角色权限）, 仅查看列表]
- Q3: "数据来源？" → [后端 API 已有, 需要 Mock 数据, 后端 API 待开发先用 Mock]
