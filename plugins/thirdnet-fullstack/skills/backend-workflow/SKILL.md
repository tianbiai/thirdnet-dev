...
└── {ProjectName}.Admin/                       # 解决方案根（dotnet new thirdnet-admin -o 产物）
    ...
    ├── Admin/
    │   ├── {ProjectName}.Admin.APIService/    # API 宿主（Controllers、Services、DTOs；三层各自内部按 Manager/App/Cockpit/Third/Shared 分端，Cockpit 按需）
...
4. DTO 层 — net-api-developer
   └── 按端类型放在 DTOs/{Manager,App,Cockpit,Third}/<模块>/：QueryManagerMap、CreateManagerMap、UpdateManagerMap、ItemManagerMap、DetailManagerMap（跨端共用放 DTOs/Shared/，类名无端段；Cockpit 按需）

5. Service 层 — net-api-developer
   └── 按端类型放在 Services/{Manager,App,Cockpit,Third}/：NoticeManagerService（Scoped，注入 IDbContextFactory + Cache + OperatorContext）；跨端共用逻辑抽到 Services/Shared/ 由各端注入

6. Controller 层 — net-api-developer
   └── 按端类型放在 Controllers/{Manager,App,Cockpit,Third}/：NoticeManagerController : AdminControllerBase（路由 api/manager/notice）
...
- [ ] 端特定类（Controller/Service/DTO）按端类型分目录（Manager/App/Cockpit/Third，Cockpit 按需）+ 类名带端后缀，跨端共用类放 Shared/（详见 net-api-developer「端类型分层组织」）
...
| DTO 命名 | `{Entity}{Action}{Endpoint}Map`（端特定，带 Manager/App/Cockpit/Third 段，Cockpit 按需）；跨端共用 `{Entity}{Action}Map` 放 Shared/ → 见 net-api-developer |
| 端类型分层 | Controller/Service/DTO 统一按 Manager/App/Cockpit/Third 分目录 + 类名带端后缀，共用放 Shared/（**官方命名端，Cockpit 按需；其他端如 OpenAPI/Iot 可扩展**）；前端 `api-typescript-spec` 已对齐同一套端词汇表 → 见 net-api-developer「端类型分层组织」「前后端对照」 |
