...
description: >
  Admin 管理后台前端模板安装与使用指南。模板预置完整后台与 API 管理功能，采用真实 API + Mock
  并行开发模式（环境变量切换数据来源，无需改业务代码），覆盖 npm exec --registry 安装、品牌定制、
  项目结构生成与安装后验证。当用户需要创建新的管理后台前端项目时必须使用此技能——禁止从零手动搭建。
...
- 后端 API 服务需单独安装（参见 `thirdnet-fullstack:backend-workflow` 技能），本技能仅负责前端
