...
├── App.vue                 # 根组件
│
├── api/                    # API 策略工厂模块（工程内扁平，URL 走 /api/manager/...）
│   ├── adapter.ts          # API 适配器接口
│   ├── adapter.web.ts      # Web 适配器（Axios + token 刷新队列）
│   ├── request.ts          # Axios 请求封装
│   ├── interfaces/         # 19 个接口契约文件（扁平）
│   ├── modules/            # 19 个 API 实现 + 工厂函数（扁平）
│   └── types/              # TypeScript 类型定义（扁平）
│
├── views/                  # 页面组件
│   ├── login/              # 登录页（粒子动画背景）
│   ├── welcome/            # 首页/欢迎页
...
每个 API 模块遵循统一的架构模式（工程内扁平，对齐 `api-typescript-spec` v2.4.0）：

```
api/types/{module}.ts        → 类型定义（枚举 + 出入参 interface）
api/interfaces/{module}.ts   → I{Module}Api 接口定义
api/modules/{module}.ts      → Real{Module}Api（Axios HTTP）+ create{Module}Api() 工厂
mock/api/{module}.ts         → Mock{Module}Api（本地数据）
mock/data/{module}.ts        → 纯数据导出
```

> Admin 模板属管理后台工程，URL 前缀固定为 `/api/manager/...`（由工程所属端决定，非目录维度）。前端按工程分端，工程内 `api/` 与 `mock/` 一律扁平——新建模块直接放对应顶层目录，勿再建 `manager/` 子目录。
