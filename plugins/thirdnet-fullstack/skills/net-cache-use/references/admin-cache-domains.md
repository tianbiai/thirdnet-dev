...
| 验证码 | 5 分钟 | 安全要求 |
| 在线状态 | 540 秒（3 × 心跳；默认心跳 180s） | 心跳驱动 |
...
| 缓存域 | 主要读取方法 | 用途 |
|--------|------------|------|
| `UserCache` | `GetUserInfo`/`GetUserDic`/`GetUserRoleIds`/`GetUserPermissions` | 用户信息、角色、权限；配合 `UserCacheInvalidation` 失效 |
...
| `TokenCache` | `SetTokenInvalidationTime`/`ClearTokenInvalidationTime`/`TryGetAsync`（实现 `IAccountTokenTimeCache`） | Token 失效时间。注意 `GetUserRoleIds` 在 `UserCache` 上，不在 `TokenCache` |
| `OnlineCache` | `BatchCheckOnlineStatus` | 在线状态（心跳驱动）。**例外**：`OnlineCache` 不继承 `RedisCacheManager`、无 Read-Through 语义，构造时直接注入 `IDatabase` |
