...
实体类必须是纯净 POCO，所有数据库映射通过 Fluent API 实现。唯一例外：`[DbBulk]` 特性（批量操作框架要求，详见下文「批量数据操作」与 [bulk-operations.md](references/bulk-operations.md)）。
...
│   └── → 批量数据操作（BulkCopy / COPY 二进制协议，见下节）
...
## 批量数据操作（BulkCopy / COPY 协议）

数据在**应用内存中**（`List<T>`，来自 Excel/API/外部系统）需要写入数据库时，用框架的 `PostgresqlAsyncBulk`（基于 `NpgsqlBinaryImport` 的 COPY 二进制协议），比 EF Core `SaveChanges` 逐条往返快一到两个数量级。

**命名空间**：`ThirdNet.Vibe.Common`（NuGet 包，`database/` 目录）。完整 API 参数与更多示例见 [bulk-operations.md](references/bulk-operations.md)。

| 类/接口 | 用途 |
|---------|------|
| `IDbAsyncBulk` | 批量操作核心接口，注入它即可 |
| `PostgresqlAsyncBulk` | 实现（COPY 二进制协议），框架自动注册为 Transient |
| `[DbBulk]` | 字段映射特性——**实体唯一允许的 Data Annotation** |
| `PostgresqlToCsvExporter` | 反向导出：`TableToCsv`/`SelectToCsv` |
...
>
> **注意**：`MergeToServer`/`MergeAndDeleteToServer` 的 `keys` 参数必须是主键或唯一约束字段；使用连接字符串的方法自动包含事务；大批量（>10 万条）建议分批处理。
...
| [bulk-operations.md](references/bulk-operations.md) | BulkCopy 完整 API 参数与示例（CopyToServer/MergeToServer 等） | 内存数据批量导入、同步、Excel 导入时 |
...
- **net-cache-use**: 缓存集成（View 模型与缓存配合；批量操作后手动删缓存）
