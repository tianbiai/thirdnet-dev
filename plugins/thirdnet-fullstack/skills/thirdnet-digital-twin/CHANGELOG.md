...
- **地面车位自适应布局**：`buildSurfaceParking` 从固定东侧单排改为按 boundary 的东/西/南/北停车带多行分配；车位、车辆、P 标牌和描边共享 placement，极端边界下统一缩放并保证不越界。
- **地下车库默认可见**：城市底板改为 boundary 外道路环带，园区地面在 `garages[]` footprint 留开口；默认鸟瞰可看到真实坑体，开口点击可进入既有 `setBelowView` 剖面视角，地下/地表拾取分离。
- **全方位相机**：OrbitControls 方位角显式无限制，极角放宽至 `[0.05, π-0.05]`，保留极点安全余量避免翻转。
- **暗色装饰可读性**：新增低层装饰材质与轮廓 helper；cyber/holographic/nebula 使用不受光装饰，night-realistic 使用轻微自发光，并调亮四套暗色主题的草地、树冠、广场、水景等 token。

### Docs

- 同步地下开口、停车带、相机极角和装饰材质规范；补齐 v2.16.0 floorHeight 历史条目遗漏。

## v2.16.0（2026-07-24）

### Changed

- **楼层高度加高**：`floorHeight` 默认值由 `24` 调整为 `40`，示例规格同步；楼栋、楼层分隔、楼顶设备、楼层拾取、连廊、POI 与相机取景均从该脚手架值派生。
