...
这些字段无法从任何输入中推断，必须确认：
- **标题** —— 「{园区名}」（页面/文档标题）。
- **视觉风格** —— 赛博（默认）/ 等距插画 / 写实日景 / 写实夜景，4 选 1（写入 `spec.style`；细则见 `references/styles.md`）。
...
1. **读取图像**并抽取：配色/风格（先判断最接近 4 种风格里的哪一种，定 `spec.style`；再用 `spec.tokens` 做细微覆盖匹配效果图配色）、相机角度、楼栋风格（玻璃幕墙/边缘发光/天线/屋顶类型）、屏幕界面元素（图例/POI 标记）、任何可读文字。
...
6. 写出 + 校验 spec。**首屏用 cyber 默认 + previewStyles 列 4 风格**——让用户能实时切风格看效果。

**完整范式**：见 `evals/files/generality/government-complex.json`（11F 主楼 + 5F 裙楼 + 6 个功能房间 POI + 246 地面车位 + previewStyles 4 风格）。
