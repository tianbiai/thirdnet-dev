...
这是中央 3D 场景的核心构建配方。下文 §2–§3 是**赛博风格**的详细配方，§4–§10（按类别上色、车库标牌、Legend、交互/选中、生命周期、园区环境）是**所有风格共用**的部分。**渲染器/灯光/材质/地面若选用其它风格**（真实物体 / 简约科技 / 夜间写实），见 `references/styles.md`——其它风格跳过 §3（网格着色器地面），按 `styles.md` 的 PBR/扁平材质与灯光构建，但 §4–§9 照常适用。
...
src/
  scene/
    ParkScene.ts          ← 新的赛博场景类（单例；见 §9）
    shaders/
      gridGround.glsl     ← 从技能 assets/ 拷贝（UV 适配的网格）
  data/
    <park>.ts             ← 从 Park Spec 生成（楼栋、车库、面板）
  composables/
    useSelection.ts       ← 从范例拷贝；跨组件选中单例
  components/center/
    GlobalTwin.vue        ← 挂载 <canvas>，实例化 ParkScene
```
...
## 3. 网格着色器地面  ← 赛博风格关键修复之一（其它风格跳过，按 `styles.md` 用纹理/PBR 地面）
...
- **园区地面**（`boundary.x*2 × boundary.z*2`）—— 赛博用上面的 shader 平面，其它风格按 `styles.md` 的草地/铺装/反射材质。
...
对每个 `BuildingSpec`，挤出一个盒体并按类别上色。**颜色来源所有风格一致**（token 的 `category` 映射：楼幢/地下车库）；**材质按风格替换**——赛博用下面的自发光 `MeshStandardMaterial`，真实物体/夜间写实用 PBR（玻璃 metalness≈0.9、混凝土 roughness≈0.9，见 `styles.md`），简约科技用扁平 `MeshBasicMaterial` + EdgesGeometry 描边。复用范例的幕墙纹理 + 楼层环线 + 屋顶轮廓方式（`DigitalTwin.ts:424-465, 766-890`），替换颜色来源。
...
- **`addFloorDividers(mesh, b)`**：在每个楼层边界（`y = i * floorHeight`，i=1..floors）画一条**贯穿四个立面的横向虚线**。用 `LineSegments` + `LineDashedMaterial`（需 `computeLineDistances()`），或 `Line2`/`LineMaterial`（`dashed: true`）。颜色取 token `building.dividerColor`（cyber/minimal/realistic/night 各自定义，见 `assets/themes/*.tokens.json`）。why：仅靠盒体边缘的环线远看会融成一片实心体；显式的横向虚线在等轴视角下被楼面“挂”住，立刻读出「这一层 / 那一层」。虚线（而非实线）是为了与 `addRoofOutline` 的实线轮廓区分层级、不喧宾夺主。
...
- **始终可见、不受选中影响**：这是「常驻标识」，不是悬停提示；金色高亮选中状态由 `buildFloorSlabs`（楼层拾取板，见本节末）的选中分支表达，楼名标签独立。
...
每个风格的「哪边是底、哪边是字」由风格气质决定（详见 `references/styles.md` 与各 `assets/themes/<style>.tokens.json`）：cyber 深底（`palette.void-bg`/`panel-top`）+ 亮霓虹字 + 霓虹描边；minimal 浅底（白/`tile-bg` 浅）+ 深主色字；realistic 中性浅底 + 深字；night-realistic 深底 + 自发光暖/亮字（夜间可见）。**理解 why**：标签是数字孪生的「可读层」，必须比楼栋/地面更跳——对比度不够就等于没标签。
...
- **半金字塔三角门**：在 `(b.x, b.z)` 用一个 `BufferGeometry` 拼出朝 `b.facing`（默认 `'S'`，即朝南/朝镜头）开口的半金字塔——底面贴地（宽 `b.w` × 深 `b.d`），远端向地下收缩成一条脊线，造型即「车库入口雨棚/坡道三角门」。可由 4 个三角形面（底、左坡、右坡、顶坡）+ 一条 `EdgesGeometry` 描边组成。材质按风格取 token 的 `category.garage` 色（cyber 自发光边、realistic/minimal/night-realistic 按 `styles.md`）。
...
- **去占用显示**：`spec` 里**没有** `garage.capacity/empty/occupied`（v1.3 已删除）。如果将来要在点击车库后弹占用数据，由详情面板从后台 API 拉取，不要塞回 3D 场景。
...
材质：园区地面颜色之上的沥青色 `MeshBasicMaterial`（cyber 用带轻微自发光的深蓝灰；realistic/minimal 用哑光沥青），`rotation.x = -π/2`、Y 略高于地面（如 `0.2`）。车道虚线用 `Line2`/`LineSegments`（白色 `environment.roadLine`）沿车道中线。**不进 `pickables[]`**。
...
- **不进 `pickables[]`**（车位/车都不影响楼栋 raycast）。材质按风格分支见 `references/styles.md`（cyber 自发光描边、realistic/minimal 扁平、night-realistic 自发光暖色）。
...
- **草地色块**（`GreenPatch`）：楼栋之间、道路两侧的低 `PlaneGeometry`，颜色取 `environment.grass`（cyber 暗紫青、realistic/minimal 自然草绿、night-realistic 深绿）。可叠程序化噪声纹理。
...
## 11. 兴趣点（POI）—— 类型化标记 + tooltip/popup（所有风格共用；v1.3 新增）

`spec.pois` 缺省为 `[]`（不生成任何 POI）。提供时，POI 与楼栋/单位同源——全部走 spec，是后台 API 可配置的「打点 + 提示」数据契约。`buildPOIs(spec.pois)` 在 `buildAmbiance` 之后调用，挂到同一个 `sceneGroup`。
...
- [ ] **赛博风格**：地面显示动画的**青色网格**（着色器地面）。**其它风格**：地面与材质按 `references/styles.md`（如真实物体有 PBR 反射 + 柔和阴影、夜间写实有窗户自发光 + bloom、简约为扁平描边），且**未接入** grid。
