...
- API 仅 GET/POST（网关限制）；字段统一 snake_case；DTO 后缀 `Map`（端特定 `{Entity}{Action}{Endpoint}Map`，如 `UserItemManagerMap` / `UserCreateAppMap` / `ParkItemCockpitMap`；跨端共用无端段 `{Entity}{Action}Map` 放 `Shared/`）。`{Endpoint}` ∈ 官方命名端 Manager/App/Cockpit/Third（Cockpit 按需；可扩展），与目录、命名空间、路由前缀一致
