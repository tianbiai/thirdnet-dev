...
## 默认行为（不询问·先把这两条当成固定基线）

升级时下面两件事**有默认值，不要每次问用户**——只在默认值走不通时才停下来确认。
...
**② 网络默认 = 内网优先，不通自动转外网**。所有 `--add-source` / `--registry` 先填内网地址；命令因**连接类错误**（超时 / 拒接 / DNS 失败 / 拉不到包的 401）失败时，**自动用外网地址重试一次，不要问用户**；外网也失败才停下来问。重试切外网时，在最终报告里记一句"内网不通，已切外网"。
...
确保 diff 用的是最新版工具，否则查到的是旧模板。**默认前后端工具都更新；源地址按「默认行为」第②条内网优先、连接失败自动转外网，不询问用户。**
...
工具只出对比素材，**不跑 apply、不跑 import-merge**。按「默认行为」第①条：检测到前后端两端时，分别 `cd` 进各自项目根跑下面命令，**两端都对比，不要只跑当前目录那一端**；只检测到一端则只跑那一端。
